...
export const calcGroundYOffset = async(containerWidth: number, containerHeight: number, imgUrl: string, percentageFromBottom: number) => {
    const containerRatio = containerWidth / containerHeight;
    const img = await getMeta(imgUrl);
    const w = img.width;
    const h = img.height;
    const imgRatio = w/h;
    if(imgRatio > containerRatio){ // match height
